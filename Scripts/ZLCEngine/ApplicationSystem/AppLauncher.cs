...
namespace ZLCEngine.ApplicationSystem
{
    /// <summary>
    ///     程序启动器抽象类
    ///     会先调用IManager的Init方法，后调用Load方法
    ///     <remarks>由于各个管理器都是继承自接口，为了不使用反射就将管理器注册到AppLauncher中，需要生成继承自AAppLauncher的类，并生成注册的代码</remarks>
    /// </summary>
    internal class AppLauncher : MonoBehaviour, IAppLauncher
    {
        internal static IAppLauncher Instance
        {
            get {
        ...
        {
            if (Instance != null) {
                Debug.LogError($"存在多个GameLauncher:\r\n1:{((MonoBehaviour)Instance).gameObject.name}\r\n2:{gameObject.name}");
                return;
            }
        ...
        {
            RegisterManagers();
            IResLoader loader = GetManager<IResLoader>();
            if (loader == null) {
        ...
        protected void RegisterManagers()
        {
            ...
            foreach (KeyValuePair<SType, SType> componentType in _componentTypes) {
        ...
        private void InitManagers()
        {
            ...
            foreach (KeyValuePair<Type, IManager> component in _components) {
        ...
        {
            foreach (KeyValuePair<Type, IManager> component in _components) {
                if (component.Value is ILoader loader) {
                    if (!(loader is IWindowManager))
                        yield return loader;
                }
            }
        }
        ...
        {
            IWindowManager windowManager = GetManager<IWindowManager>();
            void OnProgress(int progress, int total)
            {
                if (progress == total) {
                    ...
                    Action loadFinished = () =>
                    {
                        windowManager.Open(appConfig.firstMainWindowID);
                    };
                    windowManager.Open(appConfig.firstLoadingWindowID, new LoadingWindowModel(0, loadFinished));
                    IResLoader resLoader = GetManager<IResLoader>();
                    resLoader.EnableRecord();
                    IEnumerator<ILoader> preloaders = InitPreloaders();
    ...
    private void CheckPermission()
    {
        ...
        {
            if (_components.TryGetValue(typeof(M), out IManager instance)) {
                return (M)instance;
            }
            return default(M);
        }
        ...
        {
            foreach (KeyValuePair<Type, IManager> kv in _components) {
                kv.Value.Dispose();
            }
            _components.Clear();
        }
