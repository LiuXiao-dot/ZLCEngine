...
namespace ZLCEngine.EventSystem.MessageQueue
{
    ...
    public sealed class MQManager : IManager
    {
        ...
        public void Init()
        {
            GameObject mQGameObject = new GameObject("MQs");
            Transform parent = mQGameObject.transform;
            MQConfigSO mqConfigSO = MQConfigSO.Instance;
            void CreateMainMQ(MQConfig[] mqs)
            {
                foreach (MQConfig mqConfig in mqs) {
                    GameObject mq = new GameObject(mqConfig.name, typeof(MainThreadMQ));
                    MainThreadMQ mqComponent = mq.GetComponent<MainThreadMQ>();
            ...
            {
                foreach (MQConfig mqConfig in mqs) {
                    GameObject mq = new GameObject(mqConfig.name, typeof(ChildThreadMQ));
                    ChildThreadMQ mqComponent = mq.GetComponent<ChildThreadMQ>();
                    mqComponent.id = mqConfig.id;
                    AddMQ(mqConfig.id, mqComponent);
                    mq.transform.SetParent(parent);
                }
